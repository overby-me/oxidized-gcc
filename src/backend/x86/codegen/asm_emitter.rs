            ...
            match val {
                Operand::Value(v) => {
                    if let Some(slot) = self.state.get_slot(v.0) {
                        ...
                        self.state
                            .emit_fmt(format_args!("    {} {}(%rbp)", fld_instr, slot.0));
                    }
                }
                Operand::Const(c) => {
            ...
            Operand::Value(v) => {
                if let Some(slot) = self.state.get_slot(v.0) {
                    if self.state.is_alloca(v.0) {
                        ...
                        self.state.emit_fmt(format_args!(
                            "    {} {}(%rbp), {}",
                            load_instr, slot.0, dest_reg_str
                        ));
                    }
                }
            }
        ...
        if matches!(op.kind, AsmOperandKind::X87St0 | AsmOperandKind::X87St1) {
            if let Some(slot) = self.state.get_slot(ptr.0) {
                ...
                if self.state.is_direct_slot(ptr.0) {
                    self.state
                        .emit_fmt(format_args!("    {} {}(%rbp)", fstp_instr, slot.0));
                } else {
        ...
        if let AsmOperandKind::ConditionCode(ref cond) = op.kind {
            ...
            if let Some(slot) = self.state.get_slot(ptr.0) {
                ...
                if self.state.is_alloca(ptr.0) {
                    ...
                    self.state
                        .emit_fmt(format_args!("    movq %{}, {}(%rbp)", reg, slot.0));
                } else {
        ...
        if let Some(slot) = self.state.get_slot(ptr.0) {
            if is_xmm {
                ...
                if self.state.is_alloca(ptr.0) {
                    ...
                    let reg64 = Self::reg_to_64(reg);
                    self.state
                        .emit_fmt(format_args!("    movq %{}, {}(%rbp)", reg64, slot.0));
                }
            } else {
                // Non-alloca: slot holds a pointer, store through it.
                let scratch = if reg != "rcx" { "rcx" } else { "rdx" };
